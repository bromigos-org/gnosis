...
- The multi-hop item-budget multiplier is 1 (off) by default (`GNOSIS_MULTI_HOP_BUDGET_MULTIPLIER`).
...
- `GNOSIS_MULTI_HOP_BUDGET_MULTIPLIER` (default `1` = off, max 5) expands the long-term fact item budget for multi-hop-routed context reads: with adaptive routing on and the router classifying a query multi-hop, the request's `max_items` cut is multiplied by this factor before rendering. Measured motivation: LOCOMO multi-hop misses are dominated by cross-session *enumerations* ("what desserts has Maria made?" retrieved 1 of 2) — the answer's facts exist and rank in the dense pool but fall below the budget cut, a retrieval coverage gap that deeper graph traversal measurably does not fix. Applies to the budget cut only; ranking, fusion, and the graph-reserve split are unchanged, and every non-multi-hop route keeps the request budget as-is.
