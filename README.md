...
- `GNOSIS_ENTITY_GROUPED_RENDERING_ENABLED` groups retrieved facts under per-entity `####` headers on multi-hop- and aggregative-routed context reads only (default `false`, byte-identical while off; requires `GNOSIS_ADAPTIVE_ROUTING_ENABLED` to fire). Query-named entities sort first. Measured motivation: LOCOMO Run 21 proved the enumeration CoN clause alone does not repair list-shaped multi-hop misses; GRAVITY (arXiv 2605.01688) attributes its largest multi-hop gains to entity-profile anchoring rather than unstructured text.
